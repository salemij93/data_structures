 ...
 * </p><p>
 * Classnames that end in "Node" are drawn using rounded rectangles by default.  
 * This does not work until the class is loaded; thus, null references may show up 
 * as inline fields for a while, until the class is loaded.
 * </p>
...
//	}
	...
	public static void showBuiltInObjects (boolean value) {
		if (value) showNodesAsRegularObjects ();
	...
	public static void graphvizNodeBoxAttributes (String value) {
		String v = (value == null || "".equals (value)) ? "" : "," + value;
		GRAPHVIZ_NODE_BOX_ATTRIBUTES = v;
	}
	protected static String GRAPHVIZ_NODE_BOX_ATTRIBUTES = ",shape=box,style=\"rounded\",color=purple";
	/**
	...
	public static void graphvizNodeArrowAttributes (String value) {
		String v = (value == null || "".equals (value)) ? "" : "," + value;
		GRAPHVIZ_NODE_ARROW_ATTRIBUTES = v;
	}
	protected static String GRAPHVIZ_NODE_ARROW_ATTRIBUTES = ""; 
	// Intention was to set this to ",constraint=false", but this creates errors in running DOT on the resulting gv file.	
	/**
	 ...
	 * Any classname with value as a suffix will be treated as a Node.
	 * Default value includes "Node".
	 */
	public static void graphvizAddNodeClass (String value) {
		GRAPHVIZ_NODE_CLASS.add (value);
	}
	/**
	 * Remove a node class.
	 *
	 * @see graphvizAddNodeClass
	 */
	public static void graphvizRemoveNodeClass (String value) {
		GRAPHVIZ_NODE_CLASS.remove (value);
	}
	/**
	...
	public static void showNodesAsRegularObjects () {
		GRAPHVIZ_NODE_CLASS = new ArrayList<> ();
	}
	protected static ArrayList<String> GRAPHVIZ_NODE_CLASS;
	static {
		GRAPHVIZ_NODE_CLASS = new ArrayList<> ();	
		GRAPHVIZ_NODE_CLASS.add ("Node");
	}
	/**
	...
	public static void showFieldNamesInNodeLabels (boolean value) {
		GRAPHVIZ_SHOW_FIELD_NAMES_IN_NODE_LABELS = value;
	}
	protected static boolean GRAPHVIZ_SHOW_FIELD_NAMES_IN_NODE_LABELS = false;
	/**
	...
	public static void showFieldNamesInNodeArrows (boolean value) {
		GRAPHVIZ_SHOW_FIELD_NAMES_IN_NODE_ARROWS = value;
	}
	protected static boolean GRAPHVIZ_SHOW_FIELD_NAMES_IN_NODE_ARROWS = true;
	/**
	...
	private static Map<String, Connector.Argument> setMainArgs (LaunchingConnector conn, String[] args) {		
		/* TESTING CODE
		// from http://www2.sys-con.com/itsg/virtualcd/java/archives/0603/loton/index.html#s1
		...
		System.err.print (arguments);
		*/
		// This may be useful: http://pages.cs.wisc.edu/~horwitz/java-docs/guide/jpda/conninv.html#Invocation
		/* for a raw socket we need
		 * -Xdebug -Xnoagent -Djava.compiler=NONE -Xrunjdwp:transport=dt_socket,address=127.0.0.1:8000

		 */
		// get the connector argument for the program's main() method
		Map<String, Connector.Argument> connArgs = conn.defaultArguments ();
		//System.err.println(connArgs);
		...
		//TODO: Perhaps can add an IP address here?
		//TODO: Need to do something because Java 9 modules does not let me override com.sun.tools.jdi.SocketTransportService$SocketListenKey as before
		//TODO: As a result, the debugger won't start when machine running eclipse has a hostname that is not in DNS.
		//Probably need to use the RAW connector...  
		//The "easy" connector only works with hostname is found in one of the following:
...
/* private static */class Format {
	...
	public static String valueToStringShort (Value value) {
		...
		if (value instanceof PrimitiveValue) return value.toString ();
		if (Trace.SHOW_STRINGS_AS_PRIMITIVE && value instanceof StringReference) return value.toString ();
		if (Trace.SHOW_BOXED_PRIMITIVES_AS_PRIMITIVE && isWrapper (value.type ())) return value.toString (); //wrapperToString ((ObjectReference) value);
...
//	}
	...
	private static void processValueStandalone (String gvSource, String arrowAttributes, String fieldName, Value val, PrintWriter out, Set<ObjectReference> visited) {
		...
		if (isNode (objRef.type())) {
			arrowAttributes = arrowAttributes + Trace.GRAPHVIZ_NODE_ARROW_ATTRIBUTES;
		}
	...
	private static boolean objectHasNonNodeReferences (List<Field> fs, ObjectReference obj) {
		for (Field f : fs) {
			if (isNode (f)) continue;
			if (canIgnoreObjectField (f)) continue;
			if (canTreatAsPrimitive (obj.getValue (f))) continue;
			return true;
		}
		return false;
	}
	...
	private static void labelObjectWithSomePrimitiveFields (ObjectReference obj, List<Field> fs, PrintWriter out) {
		...
		for (Field f : fs) {
			...
			if (!isNode (f) && !canIgnoreObjectField (f)) {
	...
	private static void labelNodeWithSomePrimitiveFields (ObjectReference obj, List<Field> fs, PrintWriter out) {
		...
		String sep = "";
		for (Field f : fs) {
			String name = Trace.GRAPHVIZ_SHOW_FIELD_NAMES_IN_NODE_LABELS ? f.name () + " = " : "";
			if (!isNode (f) && !canIgnoreObjectField (f)) {
				if (processValueInline (Trace.GRAPHVIZ_SHOW_NULL_FIELDS, sep + name, obj.getValue (f), out)) sep = ", ";
			}
		}
		String cabs = objectAttributeMap.get (obj.type ().name ());
		out.println ("\"" + Trace.GRAPHVIZ_NODE_BOX_ATTRIBUTES + (cabs == null ? "" : "," + cabs) + "];");
	}
	private static int nullId = 0;
	private static void processNodeStandalone (boolean srcIsNode, boolean srcHasNonNodeReferences, String gvSource, String fieldName, Value val, PrintWriter out, Set<ObjectReference> visited) {
		String arrowAttributes = Trace.GRAPHVIZ_OBJECT_ARROW_ATTRIBUTES;
		if (!srcIsNode) {
			arrowAttributes = arrowAttributes + Trace.GRAPHVIZ_NODE_ARROW_ATTRIBUTES;
		}
		if (srcIsNode && !srcHasNonNodeReferences && !Trace.GRAPHVIZ_SHOW_FIELD_NAMES_IN_NODE_ARROWS) {
			fieldName = "";
		}
		if (val == null) {
			String id = "null__" + nullId;
			nullId++;
			out.print (id + "[shape=\"point\"];");			
			out.println (gvSource + " -> " + id + "[label=\"" + fieldName + "\"" + arrowAttributes + "];");
		} else {
			if (canTreatAsPrimitive (val)) throw new Error (Trace.BAD_ERROR_MESSAGE);
			ObjectReference objRef = (ObjectReference) val;
			String GvName = objectGvName (objRef);
	...
	private static boolean isNode (Type type) {
		String typeName = type.name ();
		for (String s : Trace.GRAPHVIZ_NODE_CLASS) {
			//System.err.printf ("s=%s, typeName=%s\n", s, typeName);
			if (typeName.endsWith (s)) return true;
		}
		return false;
	}
	private static boolean isNode (Field f) {
		try {
			return isNode (f.type());
		} catch (ClassNotLoadedException e) {
	...
	private static void processObject (ObjectReference obj, PrintWriter out, Set<ObjectReference> visited) {
		if (visited.add (obj)) {
			...
			if (!Trace.SHOW_BOXED_PRIMITIVES_AS_PRIMITIVE && Trace.GRAPHVIZ_SHOW_BOXED_PRIMITIVES_SIMPLY && Format.isWrapper (type)) {
				...
				if (isNode (type)) labelNodeWithSomePrimitiveFields (obj, fs, out);
				else if (objectHasPrimitives (fs, obj)) labelObjectWithSomePrimitiveFields (obj, fs, out);
				...
				if (!Format.matchesExcludePrefixShow (typeName)) {
					...
					for (Field f : fs) {
						...
						if (isNode (f)) {
							processNodeStandalone (isNode (type), objectHasNonNodeReferences (fs, obj), source, f.name (), value, out, visited);
						} else if ((!canIgnoreObjectField (f)) && (!canTreatAsPrimitive (value))) {
